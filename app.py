...
app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
    "connect_args": {
        "ssl": {} # Enables secure connection to Aiven Cloud
    }
}
...
class ReelHistory(db.Model):
    ...
    action_type = db.Column(db.String(50), nullable=False) # ISSUED, PARTIAL_RETURN, FINISHED, DAMAGED, SOLD
...
# --- System Security Roles Bypass for Login Simulation ---
@app.route('/')
def index():
...
def login_submit():
    ...
    session['role'] = username  # Set the user role dynamically (dataop1, super1 etc.)
...
def active_stock():
    ...
    locations = ['Main Store', 'Yard A', 'Production Floor', 'Viscor Line']
...
def add_stock():
    if request.method == 'POST':
        reel = Reel(
            ...
            routing_type=request.form.get('routing_type', 'Standard')
        )
        db.session.add(reel)
        db.session.commit()
...
def usage_logs():
    ...
    logs = query.all()
    locations = ['Main Store', 'Yard A', 'Production Floor', 'Viscor Line']
    return render_template('usage_logs.html', logs=logs, locations=locations, f_location=f_location, f_date=f_date, f_type=f_type, f_supplier=f_supplier)
...
def issue_reel(id):
    ...
    reel.status = 'Issued'
    if request.form.get('is_viscor') == '1':
...
@app.route('/update_stock_status/<int:id>', methods=['POST'])
@app.route('/update_status/<int:id>', methods=['POST'])
def update_stock_status(id):
    reel = Reel.query.get_or_404(id)
    status_type = request.form.get('status_type') # Damaged / Sold
