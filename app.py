...
# --- Database Configuration (ආරක්ෂිත MySQL සම්බන්ධතාවය) ---
...
class Reel(db.Model):
    ...
    status = db.Column(db.String(30), default='Full Reel') # Full Reel or Used Reel
    ...
    reel_type = db.Column(db.String(100), default='Grade A') # 'Standard' ඉවත් කර ඇත
    ...
    @property
    def type(self): return self.reel_type or 'Grade A'
    @property
...
class ReelHistory(db.Model):
    ...
    @property
    def type(self): return self.reel.reel_type if self.reel else 'Grade A'
    @property
    def used_weight(self): return self.weight_used

@app.context_processor
def inject_global_facilities():
    # අලුත් Locations ලැයිස්තුව
    return {
...
def active_stock():
    # මෙහිදී Full Reel සහ Used Reel වෙනම ලබා ගනී
...
def add_stock():
    ...
    if request.method == 'POST':
        try:
            ...
            status = request.form.get('status', 'Full Reel') # Full ද Used ද යන්න ලබාගනී
            gate_pass_number = request.form.get('gate_pass_number', '')
            gsm = request.form.get('gsm', 0, type=int)
            reel_type = request.form.get('reel_type', 'Grade A')
            store_location = request.form.get('store_location', 'VISCOR Lanka') # අලුත් Location ලබාගනී
            ...
            new_reel = Reel(
                ...
                gsm=gsm, reel_type=reel_type, store_location=store_location, supplier=supplier
            )
            db.session.add(new_reel)
            db.session.commit()
            
            initial_log = ReelHistory(reel_id=new_reel.id, usage_details=f"Stock initialized as {status} at {store_location}", action_type='INITIAL')
            db.session.add(initial_log)
            db.session.commit()
            
            flash(f"Reel {reel_number} successfully saved as {status}!", "success")
...
# ... (මීට පහළින් ඇති issue_reel, update_location, process_return වැනි අනෙකුත් routes සියල්ලම කලින් තිබූ පරිදිම පවතී)
