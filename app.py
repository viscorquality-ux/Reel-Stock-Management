...
# ==============================================================================
# ⚠️ DATABASE CONFIGURATION FOR AIVEN MYSQL & PYMYSQL WITH SSL SUPPORT
# ==============================================================================
...
class Reel(db.Model):
    ...
    type = db.Column(db.String(50), nullable=False) # Maps to Quality Type
    ...
    status = db.Column(db.String(50), default='Full Reel') # Full Reel, Partial Reel, Issued, Damaged, Sold
    ...
    @property
    def is_viscor_issued(self): return 1 if self.store_location == 'Viscor Lanka' else 0

class ReelHistory(db.Model):
    __tablename__ = 'reel_histories'
    id = db.Column(db.Integer, primary_key=True)
    reel_id = db.Column(db.Integer, db.ForeignKey('reels.id'), nullable=False)
    action_type = db.Column(db.String(50), nullable=False) # ISSUED, PARTIAL_RETURN, FINISHED, DAMAGED, SOLD, VERIFIED
    ...
    @property
    def comment_history(self): return self.notes or ""
...
def login_submit():
    ...
    flash(f"Secure Login Established as {username} successfully!", "success")
    return redirect(url_for('dashboard'))

@app.route('/logout')
def logout():
    session.pop('role', None)
    flash("Logged out from the system.", "info")
...
def add_stock():
    if request.method == 'POST':
        ...
        db.session.add(reel)
        db.session.commit()
        flash(f"New Reel {reel.reel_number} Added Successfully to Registry!", "success")
...
# 1. 🟢 NEW: Viscor Issue එකෙන් "Verified & Accept" කරන විට Active Stock එකට යැවීම
@app.route('/verify_accept/<int:id>', methods=['POST'])
@app.route('/commit_to_active/<int:id>', methods=['POST'])
def verify_accept(id):
    reel = Reel.query.get_or_404(id)
    
    # HTML Form එකෙන් එවන දත්ත පරීක්ෂා කර "Full Reel" ද "Partial Reel" ද යන්න තීරණය කිරීම
    form_data = request.form.to_dict()
    selected_status = 'Full Reel' # Default Value
    
    for key, value in form_data.items():
        if 'partial' in str(value).lower() or 'used' in str(value).lower():
            selected_status = 'Partial Reel'
            break
            
    reel.status = selected_status
    
    # අවශ්‍ය නම් Location එක වෙනස් කළ හැක (උදා: Packwell 1)
    # reel.store_location = 'Packwell 1' 
    
    history = ReelHistory(reel_id=reel.id, action_type='VERIFIED', used_weight=0, notes=f"Verified from Viscor and Committed to {selected_status}")
    db.session.add(history)
    db.session.commit()
    
    flash(f"Reel {reel.reel_number} Verified & Committed to Active Stock Successfully!", "success")
    return redirect(url_for('active_stock'))

# 2. 🔴 NEW: Viscor Issue එකෙන් "Reject/Unacceptable" කරන විට Damage Stock එකට යැවීම
@app.route('/reject_viscor/<int:id>', methods=['POST'])
def reject_viscor(id):
    reel = Reel.query.get_or_404(id)
    reel.status = 'Damaged' 
    history = ReelHistory(reel_id=reel.id, action_type='REJECTED', notes="Rejected during Viscor Verification")
    db.session.add(history)
    db.session.commit()
    flash(f"Reel {reel.reel_number} Rejected and moved to Damage & Sell Stock.", "danger")
    return redirect(url_for('damage_sell_stock'))

# අනිත් සාමාන්‍ය Processors
...
def update_status(id):
    ...
    history = ReelHistory(reel_id=reel.id, action_type=status_type.upper(), used_weight=0, notes=notes)
    db.session.add(history)
    db.session.commit()
    flash(f"Stock status updated to {status_type}.", "warning")
    return redirect(url_for('issued_stock'))
