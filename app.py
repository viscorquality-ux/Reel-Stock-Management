...
class Reel(db.Model):
    ...
    paper_name = db.Column(db.String(50), nullable=False) # Paper Type (Category)
    ...
    # නවීකරණය කරන ලද ක්ෂේත්‍ර (New Fields)
    gsm = db.Column(db.Integer, default=0)
    reel_type = db.Column(db.String(100), default='Standard') # Specification Type
...
def add_stock():
    ...
    if request.method == 'POST':
        ...
        # Form එකෙන් නව දත්ත ලබා ගැනීම
        gsm = request.form.get('gsm', 0, type=int)
        reel_type = request.form.get('reel_type', 'Standard')
        supplier = request.form.get('supplier', 'N/A') or 'N/A' # අනිවාර්ය නැත
