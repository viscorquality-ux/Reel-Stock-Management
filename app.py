...
class ActiveStock(db.Model):
    ...
    paper_name = db.Column(db.String(50)) # NEW: Paper Name Dropdown Value
    reel_condition = db.Column(db.String(20), default='Full') # NEW: Full or Used
    size = db.Column(db.String(50)) # Measurement in cm
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow) # Time stored
...
with app.app_context():
    ...
    if not User.query.first():
        ...
        db.session.add(User(username='super1', password='viscor@0000', role='Super 1'))
        db.session.add(User(username='super2', password='packwell@1111', role='Super 2'))
        db.session.commit()
...
def dashboard():
    ...
    if role in ['Data Operator 1', 'Super 1']:
    ...
    elif role in ['Data Operator 2', 'Super 2']:
...
def add_stock():
    ...
    if role in ['Super 1', 'Super 2']:
        flash("Read-Only Access: You are not allowed to add stock.", "danger")
    ...
    if request.method == 'POST':
        ...
        reel_condition = request.form.get('reel_condition')
        ...
        stock = ActiveStock(
            reel_no=reel_no, paper_name=paper_name, reel_condition=reel_condition,
            size=request.form.get('size'), gsm=request.form.get('gsm'),
...
def issue_reel(id):
    ...
    if role in ['Super 1', 'Super 2']:
        flash("Read-Only Access: You cannot issue reels.", "danger")
        return redirect(url_for('active_stock'))
...
def verify_viscor_issue(id):
    ...
    if role not in ['Admin', 'Data Operator 1']:
        flash("Unauthorized Control! Only Admin or Data Operator 1 can verify.", "danger")
...
def process_return():
    if 'username' not in session: return redirect(url_for('login'))
    if session.get('role') in ['Super 1', 'Super 2']:
        flash("Read-Only Access: Cannot process returns.", "danger")
    ...
    ret_log = ReturnReelLog(
        reel_no=stock.reel_no, paper_name=stock.paper_name, size=stock.size, gsm=stock.gsm, 
        type=stock.type, supplier=stock.supplier, previous_weight=stock.weight, 
        returned_weight=new_weight, consumption=consumption, gate_pass=stock.gate_pass, 
        store_location=stock.store_location, sr_no=stock.sr_no
    )
    usage_log = UsageReelLog(
        reel_no=stock.reel_no, paper_name=stock.paper_name, size=stock.size, gsm=stock.gsm, 
        type=stock.type, supplier=stock.supplier, used_weight=consumption, 
        gate_pass=stock.gate_pass, store_location=stock.store_location, 
        sr_no=stock.sr_no, action_type="PARTIAL_RETURN"
    )
                             
    stock.weight = new_weight
    stock.status = 'Active'
    stock.reel_condition = 'Used' # Changing condition to Used automatically
    db.session.add(ret_log)
    db.session.add(usage_log)
    db.session.commit()
    flash(f"Reel returned with {consumption}kg consumption. Moved to Used Reels.", "success")
    return redirect(url_for('issued_stock', print_reel_no=reel_no))

@app.route('/get_usage_history/<string:reel_no>')
def get_usage_history(reel_no):
    logs = UsageReelLog.query.filter_by(reel_no=reel_no).order_by(UsageReelLog.logged_at.desc()).all()
    history = []
    for log in logs:
        history.append({
            'action': log.action_type,
            'weight_used': log.used_weight,
            'date': log.logged_at.strftime('%Y-%m-%d %I:%M %p'),
            'sr_no': log.sr_no,
            'gate_pass': log.gate_pass
        })
...
def finish_reel(id):
    if 'username' not in session: return redirect(url_for('login'))
    if session.get('role') in ['Super 1', 'Super 2']:
        flash("Read-Only Access: Cannot modify stocks.", "danger")
        return redirect(url_for('issued_stock'))

    stock = ActiveStock.query.get_or_404(id)
    stock.status = 'Finished'
    usage_log = UsageReelLog(
        reel_no=stock.reel_no, paper_name=stock.paper_name, size=stock.size, 
        gsm=stock.gsm, type=stock.type, supplier=stock.supplier, 
        used_weight=stock.weight, gate_pass=stock.gate_pass, 
        store_location=stock.store_location, sr_no=stock.sr_no, action_type="FINISHED"
    )
    db.session.add(usage_log)
    db.session.commit()
...
@app.route('/add_manual_finished', methods=['POST'])
def add_manual_finished():
    if 'username' not in session: return redirect(url_for('login'))
    if session.get('role') in ['Super 1', 'Super 2']:
        flash("Read-Only Access: Cannot modify stocks.", "danger")
    ...
    usage_log = UsageReelLog(
        reel_no=stock.reel_no, paper_name=stock.paper_name, size=stock.size, 
        gsm=stock.gsm, type=stock.type, supplier=stock.supplier, 
        used_weight=stock.weight, gate_pass='-', store_location=stock.store_location, 
        sr_no=stock.sr_no, action_type="MANUAL_FINISHED"
    )
    db.session.add(usage_log)
    db.session.commit()
    flash("Finished stock manually added successfully!", "success")
    return redirect(url_for('finished_stock'))
...
def finished_stock():
    ...
    return render_template('finished_stock.html', stocks=stocks, locations=LOCATIONS)
...
def update_location(id):
    ...
    role = session.get('role')
    if role in ['Super 1', 'Super 2']:
        flash("Read-Only Access: Cannot update locations.", "danger")
        return redirect(url_for('active_stock'))
...
def delete_stock(id):
    ...
    role = session.get('role')
    if role in ['Super 1', 'Super 2']:
        flash("Read-Only Access: Cannot delete stocks.", "danger")
        return redirect(url_for('active_stock'))

    if role in ['Admin', 'Data Operator 1', 'Data Operator 2']:
        stock = ActiveStock.query.get_or_404(id)
...
def generate_qr(reel_no):
    ...
    qr_data = f"REEL:{stock.reel_no}|P:{stock.paper_name}|SIZE:{stock.size}cm|GSM:{stock.gsm}|WGT:{stock.weight}|LOC:{stock.store_location}"
