...
# --- Case-Insensitive Smart Role Helper ---
# HTML වල 'super1' සහ 'Super 1' ලෙස ඇති අසමූහිතා මඟහැරවීමට භාවිතා කරයි
class SmartRole(str):
...
class Reel(db.Model):
    ...
    gsm = db.Column(db.Integer, default=150)
    supplier = db.Column(db.String(100), default='N/A')
    ...
    # HTML templates වල විවිධ නම් වලින් කැඳවන variables එකම ඩේටාබේස් ෆීල්ඩ් එකකට මැප් කිරීම
    @property
...
class ReelHistory(db.Model):
    ...
    action_type = db.Column(db.String(50), default='LOG') # 'INITIAL', 'ISSUE', 'PARTIAL RETURN', 'FINISHED'
    timestamp = db.Column(db.DateTime, default=lambda: datetime.now(colombo_tz))

    # usage_logs.html සඳහා අවශ්‍ය Mapping Properties
    @property
    ...
    @property
    def type(self): return self.reel.paper_name if self.reel else ''
    @property
    def used_weight(self): return self.weight_used

# --- Context Processor (Global Variables) ---
# Modals සහ Filters වලදී variable missing errors වැළැක්වීමට නිරන්තරයෙන්ම දත්ත සැපයීම
...
def handle_session_and_security():
    # Role එක SmartRole එකක් බවට පත් කිරීම
...
def update_location(id):
    ...
    return redirect(request.referrer or url_for('issued_stock'))
