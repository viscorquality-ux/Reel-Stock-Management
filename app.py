...
# ==============================================================================
# ⚠️ DATABASE CONNECTION FIXED (PyMySQL Driver added & Syntax line split)
# ==============================================================================
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://avnadmin:AVNS_gHRTw4Hzio_XlhXcm7d@mysql-3e9936af-viscorquality-0270.g.aivencloud.com:28643/defaultdb?ssl-mode=REQUIRED'
...
# ==============================================================================
# 📝 FIX 1: MARK FINISHED කළ පසු FINISHED & USAGE වෙත REDIRECT කිරීම
# ==============================================================================
...
def finish_reel(id):
    ...
    log = ReelHistory(
        reel_id=reel.id, 
        usage_details=f"Marked 100% finished by production line. Remaining consumed: {used_weight}kg", 
    ...
    return redirect(url_for('finished_usage_stock')) # Finished Stock වෙත Redirect වේ.

# ==============================================================================
# 📝 FIX 3: PARTIAL RETURN CONSUMPTION හිස්ටරි LOGS ලියා තැබීම
# ==============================================================================
...
def process_return():
    ...
    # වැයවූ බර (Consumption) ගණනය කිරීම
    ...
    # එකම Reel එක කීප වතාවක් ආවත් වෙන වෙනම Consumption එක වෙලාවත් සමඟ සටහන් වේ.
    log = ReelHistory(
        reel_id=reel.id, 
        usage_details=f"Partial Return - Issued: {old_weight}kg | Returned: {returned_weight}kg", 
        weight_used=used_amount, 
        action_type='PARTIAL RETURN'
    )
    db.session.add(log)
    db.session.commit()
    flash(f'Partial return registered. Consumed {used_amount}kg logged.', 'info')
    return redirect(url_for('active_stock'))
...
def finished_usage_stock():
    ...
    # මුළු එකතුව (Total Weight) නිවැරදිව ඉතිහාස සටහන් වලින් ගණනය කිරීම
    total_finished_weight = 0.0
    for r in finished_reels:
        total_finished_weight += sum(h.weight_used for h in r.histories if h.action_type in ['PARTIAL RETURN', 'FINISHED'])
...
# ==============================================================================
# 📝 FIX 2: DAMAGE & SELL STOCK ROUTES (NOT FOUND ERROR FIX)
# ==============================================================================
...
def mark_damage_sell(id):
    reel = Reel.query.get_or_404(id)
    status_type = request.form.get('status_type') # 'Damaged' or 'Sold'
    notes = request.form.get('notes', '')
    
    old_status = reel.status
    reel.status = status_type
    
    log = ReelHistory(
        reel_id=reel.id,
        usage_details=f"Status changed from {old_status} to {status_type}. Notes: {notes}",
        weight_used=0.0,
        action_type=status_type.upper()
    )
    db.session.add(log)
    db.session.commit()
    flash(f"Reel {reel.reel_number} successfully marked as {status_type}.", "success")
