...
def add_stock():
    ...
    if request.method == 'POST':
        try:
            ...
            if existing_reel:
                flash(f"Error: Reel Number '{reel_number}' already exists! Duplicate entries not allowed.", "danger")
                return redirect(url_for('add_stock'))
...
# ==============================================================================
# 🟢 UPDATED FIXED ROUTES: ALL 404 ALIASES INCLUDED
# ==============================================================================

# 1. Viscor Lanka Line එකෙන් Active Stock එකට Verify කර භාරගැනීම
...
def verify_accept(id):
    ...
    db.session.add(log)
    db.session.commit()
    flash(f"Reel {reel.reel_number} successfully verified and committed to Active Stock.", "success")
    return redirect(url_for('viscor_issue'))

# 2. Active Stock එකෙන් රීල් එකක් Viscor Lanka Line එකට යැවීම
...
def send_to_viscor(id):
    ...
    db.session.add(log)
    db.session.commit()
    flash(f"Reel {reel.reel_number} successfully sent to Viscor Line.", "success")
    return redirect(url_for('active_stock'))

# 3. රීල් එකක තත්ත්වය Damaged / Sold / Finished ලෙස වෙනස් කිරීම (Fixed 404 for mark_damage_sell)
...
def update_status(id):
    ...
    # Form එකෙන් කෙලින්ම නොලැබුනහොත් Default එක Damaged ලෙස ගනී
    if not status_type:
        status_type = 'Damaged'
        
    notes = request.form.get('notes', 'Updated via Issued Stock Registry Screen')
    ...
    if status_type in ['Damaged', 'Sold', 'Finished']:
        ...
        flash("Invalid status type specification.", "danger")
        
    return redirect(request.referrer or url_for('active_stock'))

# 4. Damaged සහ Sold කරන ලද රීල් ලැයිස්තුව පෙන්වන පිටුව
