...
# ඔබගේ Aiven Database URL එක මෙතනට දාන්න (උදා: postgresql://user:pass@host:port/dbname)
# දැනටමත් එය සකසා ඇත්නම් එයම භාවිතා කරන්න.
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///reel_management.db' # මෙය ඔබගේ Aiven URL එකට වෙනස් කරන්න
...
def handle_session_and_security():
    ...
    allowed_routes = ['login', 'login_submit', 'static', 'reset_db'] # reset_db එකට අවසර දී ඇත
    if request.endpoint not in allowed_routes and 'role' not in session:
        return redirect(url_for('login'))

# --- AIVEN DATABASE RESET ROUTE (500 Error Fix) ---
# මෙය Run කළ විට ඔබගේ Aiven Database එකේ පරණ Tables මැකී අලුතින්ම හැදෙනු ඇත.
@app.route('/reset_db')
def reset_db():
    db.drop_all()
    db.create_all()
    flash("Aiven Database Tables successfully recreated! The 500 error should be fixed now.", "success")
    return redirect(url_for('login'))
...
def add_stock():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.rollback()
            # මීට පස්සේ error එකක් ආවොත් ඒක පැහැදිලිව පෙන්වයි.
            flash(f"Database Error! Please go to /reset_db to update your Aiven Database. Details: {str(e)}", "danger")
